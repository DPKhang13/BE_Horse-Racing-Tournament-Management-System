...
public class HorseController {
    ...
    @Operation(summary = "Get horse count", description = "Lấy tổng số ngựa.")
    ...
    @Operation(summary = "Get all horses", description = "Lấy danh sách tất cả ngựa trong hệ thống.")
    @GetMapping("/get-all")
    public ResponseEntity<List<HorseListResponse>> getAllHorses() {
        return ResponseEntity.ok(horseService.getAllHorses());
    }

    @Operation(summary = "Get horse ranking", description = "Lấy bảng xếp hạng ngựa theo ranking points, total wins và tên ngựa.")
    @GetMapping("/ranking")
    public ResponseEntity<List<HorseRankingResponse>> getHorseRanking() {
        return ResponseEntity.ok(horseService.getHorseRanking());
    }

    @Operation(summary = "Get horse by id", description = "Lấy thông tin chi tiết của một ngựa theo horse id.")
    ...
    @Operation(summary = "Delete horse", description = "Xóa một ngựa theo horse id.")
