...
public class RaceSchedulesController {
    ...
    @Operation(
            ...
            description = "Admin tao mot ngay lich thi dau cho tournament."
    )
    ...
    public ResponseEntity<TournamentScheduleResponse> createSchedule(
        ...
        return ResponseEntity
                ...
                .body(raceService.createSchedule(tournamentId, request));
    }

    @GetMapping("/tournaments/{tournamentId}/get-schedule-list")
    public ResponseEntity<List<TournamentScheduleResponse>> getSchedulesByTournament(
            @PathVariable Integer tournamentId
    ) {
        return ResponseEntity.ok(
                raceService.getSchedulesByTournament(tournamentId)
        );
    }

    @GetMapping("/schedules/get-schedule/{scheduleId}")
    public ResponseEntity<TournamentScheduleResponse> getScheduleById(
            @PathVariable Integer scheduleId
    ) {
        return ResponseEntity.ok(
                raceService.getScheduleById(scheduleId)
        );
    }
    ...
    public ResponseEntity<TournamentScheduleResponse> updateSchedule(
        ...
        return ResponseEntity.ok(
                raceService.updateSchedule(scheduleId, request)
        );
    }

    @DeleteMapping("/schedules/delete-schedule/{scheduleId}")
    public ResponseEntity<Map<String, String>> deleteSchedule(
            @PathVariable Integer scheduleId
    ) {
        raceService.deleteSchedule(scheduleId);

        return ResponseEntity.ok(
                Map.of("message", "Schedule deleted successfully")
        );
    }

    @Operation(
            summary = "Create race",
            description = "Admin tao race trong mot schedule da thuoc tournament."
    )
    ...
    public ResponseEntity<RaceResponse> createRace(
        ...
        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(raceService.createRace(scheduleId, request));
    }
    ...
    public ResponseEntity<Map<String, String>> cancelRace(
            @PathVariable Integer raceId
    ) {
        raceService.cancelRace(raceId);

        return ResponseEntity.ok(
                Map.of("message", "Race cancelled successfully")
        );
    }
}
